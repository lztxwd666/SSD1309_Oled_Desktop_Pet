...
impl IpMonitor {
    pub fn new(poll_secs: u64) -> Self {
        Self {
            last_poll: None, last_ip: None,
            known_ifaces: list_iface_names().unwrap_or_default(), poll_secs,
        }
    }
}
...
impl super::EventSource for IpMonitor {
    fn poll(&mut self, notifier: &mut Notifier) {
        ...
        if !new_iface && self.last_ip.as_deref() == Some(&entry) { return; }
...
fn get_active_ipv4() -> Option<String> {
    ...
    while !current.is_null() {
        ...
        if name == "lo" || ifa.ifa_addr.is_null() { current = ifa.ifa_next; continue; }
        // SAFETY: ifa_addr 非空，sa_family 字段始终有效。
        if unsafe { (*ifa.ifa_addr).sa_family } != libc::AF_INET as u16 { current = ifa.ifa_next; continue; }
        if !is_up(&name) { current = ifa.ifa_next; continue; }
        ...
        if o[0] == 169 && o[1] == 254 { current = ifa.ifa_next; continue; }
...
fn list_iface_names() -> Option<HashSet<String>> {
    fs::read_dir("/sys/class/net").ok().map(|d| {
        d.flatten().map(|e| e.file_name().to_string_lossy().to_string())
            .filter(|n| n != "lo").collect()
    })
}
