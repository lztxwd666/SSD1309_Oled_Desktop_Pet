...
pub fn is_night(begin: u8, end: u8) -> bool {
    ...
    if unsafe { libc::localtime_r(&ts.tv_sec, &mut tm) }.is_null() { return false; }
    let h = tm.tm_hour as u8;
    if begin <= end { h >= begin && h < end } else { h >= begin || h < end }
}

/// 检查配置文件是否更新，若是则重载热更新项。
pub fn reload_if_changed(rt: &mut super::config::RuntimeConfig, cfg_mtime: &mut i64) {
    let paths = ["configs/settings.toml", "Oled_Desktop_Pet/configs/settings.toml"];
    let found = paths.iter().find(|p| std::path::Path::new(p).exists());
    let path = match found { Some(p) => *p, None => return };
    let mt = match file_mtime(path) { Some(m) => m, None => return };
    if mt == *cfg_mtime { return; }
    *cfg_mtime = mt;
    ...
    let table = match raw.parse::<toml::Table>() {
        Ok(t) => t,
        Err(e) => { eprintln!("[配置] 解析错误: {e}"); return; }
    };
